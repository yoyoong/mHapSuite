...
public class Convert {
    ...
    private boolean convertPat(BufferedWriter bufferedWriter) throws Exception {
        ...
        while ((patLine = bufferedReader.readLine()) != null && !patLine.equals("")) {
            ...
            if (args.isFilterFlag()) {
                if (cpgInfoStr.contains(".")) {
                    continue;
                }

                Integer startPos = cpgPosList.get(startLine - 1);
                Integer endPos = cpgPosList.get(startLine + cpgInfoStr.length() - 2);

                String cpgStr = ""; // cpg string in format of 0/1
                for (char cpg : cpgInfoStr.toCharArray()) {
                    if (cpg == 'C') {
                        cpgStr += "1";
                    } else if (cpg == 'T') {
                        cpgStr += "0";
    ...
    private boolean checkArgs() {
        ...
        if (args.isPat()) {
            ...
            if (!args.getInputFile().endsWith(".bam") && !args.getInputFile().endsWith(".sam")) {
                log.error("should be input bam/sam file.");
                return false;
            }
        }
        if (!args.getMode().equals("BS") && !args.getMode().equals("TAPS")) {
            log.error("-m opt should be specified as BS or TAPS. Current: " + args.getMode());
            return false;
        }
